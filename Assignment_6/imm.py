...
# local
from discretebayes import discrete_bayes
...
class IMM(Generic[MT]):
    ...
    def reduce_mixture(
        ...
        # print("ZIPPITY ZIP", np.array(zip(*immstate_mixture.components)))
        """ Wonky tonky
        """
        # TODO !!!

        mode_states = np.sum(np.array(zip(*immstate_mixture.components))
    ...
    def gate(
        ...
        gated_per_mode = [f.gate(z, ekfstate, sensor_state=sensor_state,
                                 gate_size_square=gate_size_square) for f, ekfstate in zip(self.filters, immstate.components)]
    ...
    def _(self, init: dict) -> MixtureParameters[MT]:
        ...
        assert np.allclose(
            weights.sum(), 1), "Mode probabilities must sum to 1 for"
