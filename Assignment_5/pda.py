...
class PDA(Generic[ET]):  # Probabilistic Data Association
    ...
    def gate(
        ...
        *,
        ...
        # some for loop over elements of Z using self.state_filter.        
    ...
    def loglikelihood_ratios(
        ...
        *,
        ...
        ll[1:] = log_PD + [self.state_filter.loglikelihood(z_i,
                 filter_state, sensor_state=sensor_state) for z_i in Z]  # some for loop over elements of Z using self.state_filter.loglikelihood
    ...
    def conditional_update(
        ...
        *,
        ...
        conditional_update.append(
            # missed detection, just update with the prediction.
            filter_state
        )
        conditional_update.extend(
            # some loop over Z making a list of updates
    ...
    def update(
        ...
        *,
        ...
        gated = self.gate(Z=Z, filter_state=filter_state, sensor_state=sensor_state) 
        Zg = Z[gated]

        # find association probabilities
        beta = self.association_probabilities(Z=Zg, filter_state=filter_state, sensor_state=sensor_state)

        # find the mixture components
        filter_state_updated_mixture_components = self.conditional_update(Z=Zg, filter_state=filter_state, sensor_state=sensor_state) 
    ...
    def step(
        ...
        *,
        ...
        filter_state_predicted = self.predict(filter_state=filter_state, Ts=Ts) 
        filter_state_updated = self.update(Z=Z, filter_state=filter_state_predicted, sensor_state=sensor_state) 
